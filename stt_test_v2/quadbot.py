...
async def record(ctx):  # If you're using commands.Bot, this will also work.
    ...
    vc = await voice.channel.connect(cls=voice_recv.VoiceRecvClient)  # Connect to the voice channel the author is in.
...
async def speak(text: str):
    ...
    for i, (gs, ps, audio) in enumerate(generator):
        ...
        print(i)
        audio = audio.numpy()
        # audio = pyrb.time_stretch(audio, rate, 1.05)
        # audio = pyrb.pitch_shift(audio, rate, n_steps=0)
...
@bot.command()
async def test_graph(ctx, text: str):
    vchannel = ctx.author.voice.channel

    response = await graph.invoke_model(text)

    i = await speak(response["response"])
    for j in range(i + 1):
        await play_audio(vchannel, f"{j}.wav")
